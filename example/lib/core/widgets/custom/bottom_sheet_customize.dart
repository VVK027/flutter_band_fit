...
// radius: rounded corners of the modal (default 10.0)
// color: modal background (default Colors.white)
// builder: modal content; use Column with mainAxisSize: MainAxisSize.min
//
// ```dart
...
const Duration _kRoundedBottomSheetDuration = Duration(milliseconds: 800);
...
class _RoundedModalBottomSheetLayout extends SingleChildLayoutDelegate {
  ...
  BoxConstraints getConstraintsForChild(BoxConstraints constraints) {
    return BoxConstraints(
        ...
        minHeight: 0.0,
        maxHeight: constraints.maxHeight * 9.0 /10.5 );
  }
...
class RoundedCornerModalRoute<T> extends PopupRoute<T> {
  ...
  AnimationController createAnimationController() {
    //navigator.overlay
    animationController = BottomSheet.createAnimationController(vsync: this);
    return animationController;
  }
...
class RoundedModalBottomSheet<T> extends StatefulWidget {
  const RoundedModalBottomSheet({Key? key, required this.route}) : super(key: key);

  final RoundedCornerModalRoute<T> route;

  @override
  _RoundedModalBottomSheetState<T> createState() => _RoundedModalBottomSheetState<T>();
}
